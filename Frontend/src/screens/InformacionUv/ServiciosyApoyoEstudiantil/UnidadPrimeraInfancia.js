...
const studentVideos = [
  { id: 1, videoId: '2hFy0kOe_AM', title: 'Unidad de Primera Infancia' },
  // Puedes agregar más videos aquí
];

const UnidadPrimeraInfancia = ({ navigation }) => {
  const [currentIndex, setCurrentIndex] = useState(0);
  const scrollX = useRef(new Animated.Value(0)).current;
  const [playingIndex, setPlayingIndex] = useState(null);

  const email = 'programa.infancia@uv.cl';

  const onVideoPlay = (index) => setPlayingIndex(index);

  const handleEmail = () => {
    const mailtoURL = `mailto:${email}?subject=${encodeURIComponent(
      '[Unidad de Primera Infancia - UV]'
    )}&body=${encodeURIComponent('Hola, quisiera contactar con la Unidad de Primera Infancia. Gracias.')}`;
    ...
    <SafeAreaView style={[GlobalStyle.container, GlobalStyle.androidSafeArea]}>
      ...
      <View style={styles.headerContainer}>
        <View style={styles.headerRow}>
          ...
          <Text style={styles.headerTitle}>Unidad de Primera Infancia</Text>
        </View>
        <Text style={[GlobalStyle.text, styles.headerDescription]}>
          Esta Unidad pretende generar un espacio de apoyo socioafectivo y educativo
           a estudiantes que cumplen el rol de ser padres o madres.
        </Text>
      </View>
